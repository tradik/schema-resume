...
class SchemaComparator:
    ...
    def _print_summary(self, schema_fields: Set[str], context_fields: Set[str],
        ...
        print()
        # These four counts are NOT comparable with each other, and printing
        # them in a column invites exactly that reading. schema.json is counted
        # as dotted paths (`basics.name`, `work[].position`), context.jsonld as
        # bare terms (`name`, `position`), meta-schema.json as top-level
        # properties only, and the XSD as element names. Reading 126 against 94
        # and concluding the JSON-LD context covers three quarters of the schema
        # is wrong: the context maps terms, and one term covers every path that
        # ends in it.
        print("Raw counts (different units — see the coverage check below):")
        print(f"  schema.json:      {len(schema_fields)} dotted paths")
        print(f"  context.jsonld:   {len(context_fields)} terms")
        print(f"  meta-schema.json: {len(meta_fields)} top-level properties")
        print(f"  XSD:              {len(xsd_fields)} element names")

        self.print_context_coverage(schema_fields, context_fields)

    def print_context_coverage(self, schema_fields: Set[str], context_fields: Set[str]):
        """Report JSON-LD coverage in comparable units.

        A field that no context term maps produces no RDF triple — it simply
        disappears when the document is read as JSON-LD. That is the number
        worth watching, so it is computed on leaf names, which is the unit the
        context actually works in.
        """
        # `@type` and friends are JSON-LD keywords, not fields to map.
        leaves = {
            path.split('.')[-1] for path in schema_fields
            if not path.split('.')[-1].startswith(('@', '$'))
        }
        unmapped = sorted(leaf for leaf in leaves if leaf not in context_fields)

        print()
        print("JSON-LD coverage (leaf field names, the unit @context maps):")
        covered = len(leaves) - len(unmapped)
        pct = (covered / len(leaves) * 100) if leaves else 100.0
        print(f"  {covered}/{len(leaves)} mapped ({pct:.1f}%)")

        if unmapped:
            print("  Unmapped — these produce no RDF triples:")
            for leaf in unmapped:
                print(f"    - {leaf}")
        else:
            print("  ✅ every schema field has a context mapping")
