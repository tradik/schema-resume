...
# The dialect schema.json and meta-schema.json are written against. A $schema
# value has to name a dialect a validator already knows, not the document that
# happens to describe this project.
...
class SchemaLinter:
    ...
    def lint_schema_json(self):
        ...
        try:
            ...
            # $schema must name a dialect every validator recognises, so that
            # `ajv compile -s schema.json` and its equivalents work with no
            # per-consumer setup.
            if '$schema' in data:
                if data['$schema'] in KNOWN_DIALECTS:
                    self.info.append(("schema.json", f"✅ Declares dialect {data['$schema']}"))
                else:
                    self.errors.append((
                        "schema.json",
                        f"$schema must name a JSON Schema dialect (e.g. {DRAFT_07}), "
                        f"got {data['$schema']} — validators cannot resolve it"
                    ))
    ...
    def lint_meta_schema(self):
        ...
        try:
            ...
            # $schema must name a JSON Schema dialect, not this document.
            #
            # It used to be self-referential ($schema == $id), which is legal but
            # leaves no dialect anywhere in the chain: a validator following
            # $schema arrives back here and still does not know which keywords
            # apply. In practice tools just fail — plain `ajv compile -s
            # schema.json` errored with "no schema with key or ref
            # https://schema-resume.org/meta-schema.json" until every consumer
            # registered the meta-schema by hand.
            if '$schema' in data:
                if data['$schema'] in KNOWN_DIALECTS:
                    self.info.append(("meta-schema.json", f"✅ Declares dialect {data['$schema']}"))
                elif data.get('$schema') == data.get('$id'):
                    self.errors.append((
                        "meta-schema.json",
                        "$schema is self-referential — it must name a JSON Schema dialect "
                        f"(e.g. {DRAFT_07}), or no validator can resolve it"
                    ))
                else:
                    self.warnings.append((
                        "meta-schema.json",
                        f"$schema is not a recognised dialect: {data['$schema']}"
                    ))
